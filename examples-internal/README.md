...
- `simple_scheduler.py` — a naive, greedy scheduler: assign any boarding
  ...
  fairness, no awareness of gate outages beyond correctness. This is
  intentionally close to the "dumbest scheduler that still works" described
  in `CHALLENGE.md` §4 — it's the floor, not an example to aspire to.
- `better_scheduler.py` — orders voyages by *slack* (deadline minus
  ...
  fragmentation. The strategy lives entirely in the `Scheduler` class
  (`decide()`), isolated from the HTTP loop in `run()`, so it's easy to see
  exactly what a "swappable strategy" architecture (per `CHALLENGE.md` §9)
...
| Run | Cycles/sampling | overallScore | throughput | gateUtilization | arrivalSuccess | fairness | reliability |
| --- | --- | --- | --- | --- | --- | --- | --- |
